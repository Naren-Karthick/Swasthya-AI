  ...
  return (
    <div className="mx-auto max-w-3xl px-3.5 py-6 sm:py-12">
      ...
      <div className="mb-4 sm:mb-6 flex items-center justify-between gap-2">
        <button
          onClick={onBack}
          className="group inline-flex items-center space-x-1.5 sm:space-x-2 rounded-xl bg-white px-3 py-1.5 text-xs sm:text-sm font-semibold text-slate-600 hover:text-slate-900 border border-slate-200 shadow-soft-sm transition-all cursor-pointer min-h-[38px]"
        >
          <ArrowLeft className="h-3.5 w-3.5 sm:h-4 sm:w-4 transition-transform group-hover:-translate-x-1 text-teal-600 flex-shrink-0" />
          <span>Back to Home</span>
        </button>

        {/* Live Audio AI Alternative Trigger */}
        {onSwitchToVoice && (
          <button
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-5 sm:p-10 shadow-soft-lg">
        
        {/* Card Title */}
        <div className="mb-6 sm:mb-8">
          <div className="inline-flex items-center gap-1.5 rounded-lg bg-teal-50 px-2.5 py-1 text-[11px] font-bold text-teal-800 border border-teal-100 mb-2">
            <Sparkles className="h-3 w-3 text-teal-600" />
            <span>Clinical Symptom Intake</span>
          </div>
          ...
          <p className="mt-1.5 text-xs sm:text-sm text-slate-500 leading-relaxed font-medium">
            Describe all current symptoms, onset, and severity in your native language. You can type or use the microphone.
          </p>
        </div>
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            ...
            <span className="text-[10px] sm:text-[11px] font-medium text-slate-400">Click to append</span>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {quickChips.map((chip) => (
              <button
                ...
                className="group inline-flex items-center rounded-xl bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-teal-50 hover:text-teal-800 border border-slate-200 hover:border-teal-300 shadow-soft-sm active:scale-95 transition-all cursor-pointer min-h-[36px]"
              >
                <span className="text-teal-500 group-hover:scale-125 mr-1 font-bold transition-transform">+</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
            ...
            <div className="relative">
              ...
              {speechSupported ? (
                ...
                <div className="absolute right-3 bottom-3 text-[11px] font-medium text-slate-400 flex items-center gap-1">
                  ...
                  Speech API unavailable in browser
                </div>
              )}
            </div>

            {/* Real-Time Live Speech Banner when speaking */}
            {isListening && (
              <div className="mt-2 rounded-xl border border-teal-200 bg-teal-50/90 p-3 text-xs text-teal-950 font-medium flex items-center gap-2 shadow-xs animate-in fade-in">
                <span className="relative flex h-2.5 w-2.5 flex-shrink-0">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-500 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-rose-600"></span>
                </span>
                <span className="font-bold text-teal-800 flex-shrink-0">You are saying:</span>
                <span className="italic truncate font-bold text-teal-900">
                  {interimSpokenText ? `"${interimSpokenText}"` : 'Listening to your voice...'}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="grid gap-4 sm:gap-5 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/30 p-3.5 sm:p-4">
              <label htmlFor="ageGroup" className="flex items-center space-x-1.5 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">
            ...
            {/* Symptom Duration */}
            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/30 p-3.5 sm:p-4">
              <label htmlFor="duration" className="flex items-center space-x-1.5 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">
          ...
          >
            {loading ? (
              <div className="flex items-center space-x-2">
                ...
                <span>{translations.analyzing || 'Analyzing symptoms with Swasthya AI...'}</span>
              </div>
            ) : (
              <div className="flex items-center space-x-2">
